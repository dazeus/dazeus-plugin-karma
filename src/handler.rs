    ...
    match line(&evt[3]) {
        Ok(changes) => {
            ...
            for change in totals {
                let value = store_karma_change(&change, dazeus).unwrap();
...
pub fn reply_to_karma_command(evt: &Event, dazeus: &DaZeusClient) {
    let term = &evt[4].trim();
    if term != &"" {
        let karma = match KarmaValue::from_dazeus(dazeus, term) {
            Ok(karma) => karma,
    ...
    if evt.len() > 5 {
        ...
        if karmas.len() == 1 {
            ...
            if highest.len() == 1 {
                ...
                dazeus.reply(&evt, &format!("{} wins with {}", first.term, first.votes.to_string())[..], false);
            } else {
                let terms = highest.iter().map(|e| e.term.clone() ).collect::<Vec<String>>().connect(", ");
...
fn store_karma_change(change: &Karma, dazeus: &DaZeusClient) -> Result<KarmaValue, Box<Error>> {
    let property = format!("{}{}", ::karma::STORE_PREFIX, &change.term[..]);
    let current = dazeus.get_property(&property[..], Scope::any());
    let mut karma = match current.get_str("value") {
        Some(s) => try!(KarmaValue::from_str(s)),
        None => KarmaValue::new(&change.term[..]),
    };
    karma.vote(change);
    dazeus.set_property(&property[..], &karma.to_json().to_string()[..], Scope::any());
    Ok(karma)
}
...
fn retrieve_all_karmas(evt: &Event, dazeus: &DaZeusClient) -> Vec<KarmaValue> {
    ...
    for key in 5..evt.len() {
        if !karmas.iter().any(|e: &KarmaValue| e.term == &evt[key]) {
            karmas.push(match KarmaValue::from_dazeus(dazeus, &evt[key]) {
                Ok(karma) => karma,
